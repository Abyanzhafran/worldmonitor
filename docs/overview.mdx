...
World Monitor publishes a public, searchable [Data Source Catalog](https://worldmonitor.app/sources). Its manifest-derived inventory currently lists 545 active providers, 547 upstream hosts, 296 structured endpoints, and 279 news and OSINT feeds. The separate application inventory records 658 configured feed definitions, and the Telegram catalog enables 56 channels. These figures describe different inventory layers and should not be added together.
...
| [MCP server](/mcp-overview) | Claude, Cursor, and custom AI agents | Streamable HTTP with 64 tools, 6 workflow prompts, 10 interactive apps, and optional JMESPath response filtering. Catalog discovery is public. `get_sources` is the sole credential-free, daily-quota-free data tool and uses a separate fail-closed 10/minute/IP anonymous ceiling; every other data tool requires subscription access through OAuth 2.1 or a World Monitor API key. MCP entitlements and metering are documented separately from REST API-plan allowances. |
| [Official SDKs](/sdks) and [CLI](/cli) | Python, Ruby, Go, and JavaScript applications or shell automation | The relevant language runtime. `get_sources` needs no key; all other hosted data tools need a subscription API key. The zero-dependency SDKs and npm CLI support hosted and self-hosted base URLs. |
...
| [Agent discovery](/agent-discovery) | Automated onboarding and agent-native workflows | Standard `.well-known` metadata, `llms.txt`, OpenAPI assets, an MCP server card, and a catalog of 25 installable agent skills. Discovery metadata is public; `get_sources` is the one credential-free data-tool exception, and other data-bearing operations require the applicable subscription credentials. |
...
500+ curated news feeds across geopolitics, defense, energy, think tanks, and regional news (Middle East, Africa, Latin America, Asia-Pacific), backed by 547+ observed upstream hosts.

...
description: "Script country briefs, risk scores, and any of the 64 MCP tools from your shell with the official worldmonitor npm CLI — no API integration to write."
---
...
You'll get all 64 tool definitions back as JSON. You can also run `worldmonitor call get_sources` without a key: it is the sole credential-free, daily-quota-free data tool, with a separate fail-closed ceiling of 10 anonymous calls/minute/IP. Every other data-bearing `tools/call` needs a subscription API key.
...
The curated table stays small on purpose — every one of the 64 tools is reachable via `call`, and any unrecognised `--key value` becomes a tool argument, so nothing needs special wiring:
...
- **[MCP Tools Reference](/mcp-tools-reference)** — per-tool parameters, freshness budgets, and examples for all 64 tools.

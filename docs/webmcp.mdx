...
WorldMonitor still runs read-only tools and reversible dashboard view changes on these versions. `openSearch`, `open_dashboard_panel`, `set_map_view`, `set_time_range`, and `focus_country` work normally. `set_map_view`, `set_time_range`, and `focus_country` write the address bar through `history.replaceState`, producing the same share URL the page's link-copy control produces and that the dashboard restores on reload; that write is visible in the address bar and is overwritten by the next human map move.

`openCountryBrief`, `set_map_layers`, `set_map_mode`, and `open_search_result` stay gated and reject as unavailable. `openCountryBrief` can start server-side LLM generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`) that consumes the caller's daily allowance even if the browser later cancels the caller-side promise. The two layer tools persist the map-layer selection to browser storage, and enabling the AIS layer also opens a network stream. `set_map_mode` persists the 2D/3D preference and can persist a resilienceScore compatibility change. These effects can outlive cancellation, so the tools become available only on a browser that supplies the target-side signal.
...
On a browser that omits the target-side invocation signal, the reversible view-state tools still run: because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands. `openCountryBrief`, `set_map_layers`, `set_map_mode`, and `open_search_result` reject as unavailable there instead. The country tool can consume daily LLM allowance after caller cancellation, while the layer tools persist selection beyond the session and `set_map_mode` persists the renderer preference. See [Availability](#availability).
...
| A call returns `target_cancellation_unsupported` | The browser accepted WebMCP but did not give the page the invocation's `AbortSignal` | Use read-only or reversible view-state tools. Do not bypass the denial for `openCountryBrief`, `set_map_layers`, `set_map_mode`, or `open_search_result`. |

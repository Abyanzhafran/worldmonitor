...
`openCountryBrief`, `set_map_layers`, and `set_map_mode` stay gated and reject as unavailable. `openCountryBrief` can start server-side LLM generation (`premiumFetch` → `/api/intelligence/v1/get-country-intel-brief` → `callLlm`) that consumes the caller's daily allowance even if the browser later cancels the caller-side promise. `set_map_layers` persists the map-layer selection to browser storage, and enabling the AIS layer also opens a network stream. `set_map_mode` persists the 2D/3D preference and can persist a `resilienceScore` compatibility change. `open_search_result` is a selector: it runs a view-state result and rejects persistent, quota-consuming, or external-navigation results. These effects can outlive cancellation, so they become available only on a browser that supplies the target-side signal.
...
On a browser that omits the target-side invocation signal, the reversible view-state tools still run: because such a browser cannot cancel them, an invocation you abort still completes and its visible dashboard change still lands. `openCountryBrief`, `set_map_layers`, and `set_map_mode` reject as unavailable there instead. `open_search_result` still runs a view-state result and rejects a persistent, quota-consuming, or external-navigation result. The country tool can consume daily LLM allowance after caller cancellation, while layer writes and `set_map_mode` persist beyond the session. See [Availability](#availability).
...
| A call returns `target_cancellation_unsupported` | The browser accepted WebMCP but did not give the page the invocation's `AbortSignal` | Use read-only or reversible view-state tools. Do not bypass the denial for `openCountryBrief`, `set_map_layers`, or `set_map_mode`. For `open_search_result`, choose a view-state result or wait for a host that can cancel persistent work. |

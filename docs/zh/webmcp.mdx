...
在这些版本上，WorldMonitor 仍会执行只读工具和可逆的仪表板视图变更。`openSearch`、`open_dashboard_panel`、`set_map_view`、`set_time_range` 与 `focus_country` 均可正常使用。`set_map_view`、`set_time_range` 与 `focus_country` 会通过 `history.replaceState` 写入地址栏，生成的分享 URL 会在刷新时还原。

`openCountryBrief`、`set_map_layers` 与 `set_map_mode` 会继续被拦截并返回不可用。`openCountryBrief` 可能消耗每日简报额度，`set_map_layers` 会持久化图层选择，`set_map_mode` 会持久化 2D/3D 偏好及可能的 `resilienceScore` 兼容性变更。`open_search_result` 仍会执行视图状态结果，并拒绝持久化、配额消耗或外部导航结果。这些影响可能在取消后继续存在，因此只有浏览器提供目标侧信号时才可用。
...
在没有提供目标侧调用信号的浏览器上，可逆的视图状态工具仍会执行。`openCountryBrief`、`set_map_layers` 与 `set_map_mode` 会返回不可用。`open_search_result` 仍会执行视图状态结果，并拒绝持久化、配额消耗或外部导航结果。国家简报可能消耗配额，图层写入与 `set_map_mode` 会持久化。参见[可用性](#availability)。
...
| 调用返回 `target_cancellation_unsupported` | 浏览器接受了 WebMCP，但没有把调用的 `AbortSignal` 交给页面 | 使用只读工具或可逆视图状态工具。不得绕过 `openCountryBrief`、`set_map_layers` 或 `set_map_mode` 的拒绝。对于 `open_search_result`，请选择视图状态结果，或等待能够取消持久化工作的宿主。 |

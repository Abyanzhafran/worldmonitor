...
不要猜测面板 ID、图层 ID、结果键、权益或隐藏数据。先读取当前页面状态或调用 `list_dashboard_panels`，再调用一个受限操作，检查结果和可见效果，然后继续。
...
- 仪表板运行遥测严格受限：`webmcp-registered` 记录 `toolCount`、`pageSurface` 和 API 类别；`webmcp-registration-failed` 记录工具及稳定原因；`webmcp-tool-invoked` 记录工具、结果和终态原因。仪表板搜索还可以记录查询长度、结果数及允许列表内的结果类型类别。面板目录还可以记录结果数及是否还有下一页。这些 WebMCP 专用自定义属性不得包含参数、搜索文本、结果键、返回内容、URL、招标内容或用户身份。事件仍使用 WorldMonitor 常规的 Umami 页面与会话外层信息，其中包含页面上下文，并可能与已登录的仪表板身份关联；受限路径只会省略自动内容归因属性，不会移除常规分析会话元数据。
...
| `src/services/webmcp-panel-catalog.ts` | 规范面板目录的分页、筛选、权益叠加及稳定的不可用原因。 |
| `src/App.ts` 和 `src/app/webmcp-dashboard.ts` | 启动顺序、UI 就绪门禁、销毁、仪表板上下文、面板目录绑定和人工路径操作绑定。 |

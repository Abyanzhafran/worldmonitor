...
所有付费套餐共享同一个 MCP 服务器和同样的 64 个工具。控制面板签发的 API key（`wm_…`）客户端和大多数 OAuth 上下文使用硬性默认值：**每个 UTC 日 50 次消耗配额的 `tools/call` / `resources/read` 调用**。OAuth 额度按套餐解析；API Starter 和 API Business 当前仍使用相同的每日 50 次默认值，企业 OAuth 可以不设上限。只有部署中明确列入许可名单的旧版运营方密钥不进入每日预留路径。每个已认证上下文还受 **60 次请求/分钟**限流器保护。`get_sources` 和元数据辅助工具 `describe_tool` 不计入每日配额。
...
有关完整的 64 个工具列表，请参见上方的[工具目录](#tool-catalog)；有关各工具详情，请参阅 [MCP 工具参考](/zh/mcp-tools-reference)。

## Prompt 与资源

除了 64 个工具之外，WorldMonitor 还暴露 MCP prompt 和资源，使客户端无需从零编排工具计划即可发现常见工作流并寻址稳定的数据切片。

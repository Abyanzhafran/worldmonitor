...
`get_market_data` is one of 64 tools. The rest cover:
...
2. Claude Desktop calls `tools/list` and receives 64 compressed tool descriptions (`≤120` bytes per tool). The compressed form keeps `tools/list` cheap; `describe_tool` returns the full definition on demand.

...
WorldMonitor's MCP server exists for this exact pattern. It exposes 64 tools, six prompt templates, and read-only resources through the [Model Context Protocol](https://modelcontextprotocol.io), so Claude can discover tools and call them during a conversation.
